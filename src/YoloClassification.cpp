...
state_t YoloClassification::init(int device) {
    ...
    outputLayers.append("yolov3/yolov3_head/Conv_6/Conv2D"); // 1x13x13x255
    outputLayers.append("yolov3/yolov3_head/Conv_14/Conv2D"); // 1x26x26x255
    outputLayers.append("yolov3/yolov3_head/Conv_22/Conv2D"); // 1x52x52x255
#else
    ...
    if(NULL == snpe) {
        ...
        outUBMap.add("yolov3/yolov3_head/Conv_6/BiasAdd:0", outBuf1.get()); // 1x13x13x255
        outUBMap.add("yolov3/yolov3_head/Conv_14/BiasAdd:0", outBuf2.get()); // 1x26x26x255
        outUBMap.add("yolov3/yolov3_head/Conv_22/BiasAdd:0", outBuf3.get()); // 1x52x52x255
...
std::vector<CLASSIFY_DATA> YoloClassification::doDetect(std::shared_ptr<cv::Mat> &img) {
    ...
    for (int j = 0; j < select_bbox_idxs.size(); j++) {
        ...
        if (score > mConfidenceThreshold) {
            ...
            data.rect[2] = x2 - x1;
            data.rect[3] = y2 - y1;
            data.confidence = select_score_vec2.at(idx);
